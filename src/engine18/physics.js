...
game.CollisionSolver = game.Class.extend({
    ...
    **/
    hitResponse: function(a, b) {
    	...
    	if (a.collide(b)) {
    		//determine center if shape A
    		var a_center=new game.Vector().copy(a.position);
    		if(a.shape instanceof game.Rectangle){
    			a_center.x+=a.shape.width/2;
    			a_center.y+=a.shape.height/2;
    		}
    		else if(a.shape instanceof game.Circle){
    			a_center.add(a.shape.radius);
    		}
    		//determine center if shape B
    		var b_center=new game.Vector().copy(b.position);
    		if(b.shape instanceof game.Rectangle){
    			b_center.x+=b.shape.width/2;
    			b_center.y+=b.shape.height/2;
    		}
    		else if(b.shape instanceof game.Circle){
    			b_center.add(b.shape.radius);
    		}
    		//prepare masses
    		var a_mass=a.mass+0.001;
    		var b_mass=b.mass+0.001;
    		//calculate an impulse vector based on velocity and mass of each body
    		var distanceAB=new game.Vector().copy(b_center).subtract(a_center);
    		var velocityAB=new game.Vector().copy(b.velocity).subtract(a.velocity);
    		//is distance decreasing? Then invert both speeds
    		var dDistance=new game.Vector().copy(distanceAB).multiply(velocityAB);
    		if(dDistance.x<0){
    			//execute impuls
    			var avx=a.velocity.x;
    			var bvx=b.velocity.x;
    			a.velocity.x=(avx*(a_mass-b_mass)+2*b_mass*bvx)/(a_mass+b_mass);
    			b.velocity.x=(bvx*(b_mass-a_mass)+2*a_mass*avx)/(a_mass+b_mass);
    		}
    		if(dDistance.y<0){
    			//execute impuls
    			//execute impuls
    			...
    			a.velocity.y=(avy*(a_mass-b_mass)+2*b_mass*bvy)/(a_mass+b_mass);
    			b.velocity.y=(bvy*(b_mass-a_mass)+2*a_mass*avy)/(a_mass+b_mass);
    		}
     		return true;
    	}
    	return false;
